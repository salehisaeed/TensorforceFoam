...
namespace cppflow {
   ...
   // This specialized constructor needs to be defined to create boolean tensor for TF mdoels
    template<>
    tensor::tensor(const std::vector<bool>& values, const std::vector<int64_t>& shape) 
    {
        std::vector<uint8_t> data(begin(values), end(values));
        *this = tensor(TF_BOOL, data.data(), values.size() * sizeof(bool), shape) ;
    }
